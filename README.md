...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph AUTH ["🔐  Authentication"]
        direction TB
        A1["Register / Create Account"]
        A2["Login — Email · OAuth"]
        A3["Logout / Refresh Token"]
        A4["Update Profile / Avatar"]
        A5["Search Users"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph MSG ["💬  Direct Messaging"]
        direction TB
        M1["Send Direct Message\n(SignalR)"]
        M2["Receive Real-Time Message"]
        M3["View Message History"]
        M4["Edit Message"]
        M5["Delete Message"]
        M6["Search Messages"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph ROOMS ["🏠  Chat Rooms & Groups"]
        direction TB
        R1["Create Room / Group Chat"]
        R2["Join / Leave Room"]
        R3["Send Room Message\n(SignalR)"]
        R4["View Room Members"]
        R5["Manage Room\n(Update / Delete)"]
        R6["Browse Public Rooms"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph PRESENCE ["🟢  Presence & Status"]
        direction TB
        P1["Set Online / Offline Status"]
        P2["Typing Indicator\n(Send / Stop)"]
        P3["Mark Message Read / Unread"]
        P4["View Unread Message Count"]
        P5["View Active Connections"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph MEDIA ["📎  Media & Files"]
        direction TB
        F1["Upload Image / File\nIn Chat"]
        F2["View / Download Media Etc."]
        F3["Send Media Message\n(SignalR)"]
        F4["View Room Media Gallery"]
        F5["Delete Uploaded File"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph NOTIF ["🔔  Notifications"]
        direction TB
        N1["Receive Message\nPush Notification"]
        N2["View Notification Centre"]
        N3["Mark Notifications\nRead / All Read"]
        N4["Receive Mention / Tag Alert"]
        N5["Unread Notification\nBadge (SignalR)"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph ADMINMGMT ["🛠️  Admin Management"]
        direction TB
        AD1["Manage Users\n(Suspend / Delete)"]
        AD2["View All Rooms / Messages"]
        AD3["Delete Room / Message"]
        AD4["View Platform Analytics"]
        AD5["View Audit Logs"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    subgraph SYSAUTO ["🤖  System Automation"]
        direction TB
        SA1["Auto User Online /\nOffline Detection"]
        SA2["Cleanup Expired\nMedia Files"]
        SA3["Send New Message\nPush Notification"]
        SA4["Send Platform\nNotification (Admin)"]
        SA5["View Connection\nCount (Real-Time)"]
    end
...
%% ─────────────────────────────────────────────────────────────
    ...
    classDef actor      fill:#EEEDFE,stroke:#7F77DD,color:#26215C,rx:50,font-weight:600
    classDef authNode   fill:#E1F5EE,stroke:#1D9E75,color:#04342C
    classDef msgNode    fill:#E6F1FB,stroke:#378ADD,color:#042C53
    classDef roomNode   fill:#FAEEDA,stroke:#BA7517,color:#412402
    classDef presNode   fill:#EAF3DE,stroke:#639922,color:#173404
    classDef mediaNode  fill:#FBEAF0,stroke:#D4537E,color:#4B1528
    classDef notifNode  fill:#FAECE7,stroke:#D85A30,color:#4A1B0C
    classDef adminNode  fill:#EEEDFE,stroke:#7F77DD,color:#26215C
    classDef sysNode    fill:#F1EFE8,stroke:#888780,color:#2C2C2A

    class USER,GUEST,ADMIN,SYSTEM actor
    class A1,A2,A3,A4,A5 authNode
    class M1,M2,M3,M4,M5,M6 msgNode
    class R1,R2,R3,R4,R5,R6 roomNode
    class P1,P2,P3,P4,P5 presNode
    class F1,F2,F3,F4,F5 mediaNode
    class N1,N2,N3,N4,N5 notifNode
    class AD1,AD2,AD3,AD4,AD5 adminNode
    class SA1,SA2,SA3,SA4,SA5 sysNode
```
...
|--------|-----------|
| 🟢 Teal | Authentication |
| 🔵 Blue | Direct Messaging |
| 🟡 Amber | Chat Rooms & Groups |
| 🌿 Green | Presence & Status |
| 🌸 Pink | Media & Files |
| 🟠 Coral | Notifications |
